...
function Page() {
  ...
  const fetchContactDetails = async () => {
    try {
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_BACKLINK}/dashboard/enquiry/list`,
  ...
  const handleUpdate = async () => {
    try {
      ...
      await axios.put(
        `${process.env.NEXT_PUBLIC_BACKLINK}/dashboard/enquiry/update/${id}`,
  ...
  const handleDelete = async (id) => {
    swal({
      ...
      if (willDelete) {
        try {
          await axios.delete(
            `${process.env.NEXT_PUBLIC_BACKLINK}/dashboard/enquiry/delete/${id}`,
            { withCredentials: true }
          );
          fetchContactDetails();
  ...
  return (
    <div className="container pt-3">
      <h2 style={{ color: "#007bff" }}> Booking Enquiries </h2>
        ...
        <tbody>
          {contactDetails.map((item, index) => (
            <tr key={item._id}>
              ...
              <td>{moment(item.departDate).format("DD-MM-YYYY")}</td>
